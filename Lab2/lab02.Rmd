...
bmps %>% group_by(StateAbbreviation) %>% summarise(min = min(Cost,na.rm= TRUE),mean = mean(Cost,na.rm= TRUE),median = median(Cost,na.rm= TRUE), max = max(Cost,na.rm= TRUE),standiv = sd(Cost,na.rm= TRUE))
...
bmps %>% filter(str_detect(BMPShortName, "cover"))%>% ggplot(aes(x = StateAbbreviation,y =TotalAmountCredited))+ geom_boxplot()
...
dams.stream<- st_join(dams,streams)
streamav.state<- dams.stream%>% group_by(STATE.x)%>%summarize(length.streams=mean(LengthKM, na.rm = TRUE))
damsheight.state<- dams%>% group_by(STATE)%>%summarize(ht.dams=mean(HEIGHT, na.rm = TRUE))
av.htlength<- st_join(streamav.state,damsheight.state)
av.htlength %>% ggplot(aes(x=length.streams,y=ht.dams, label=STATE.x, color=STATE.x))+geom_point(size=5)+labs(title= "Average Dam Height vs Average Stream Length for MD,VA,and PA", x= "Average Stream Length KM", y= "Average Dam Height M", color= "State Abbreviation")
...
count.stream<- st_join(counties, streams)
tot_coun<- count.stream%>% group_by (NAME10)%>% slice_max(order_by=LengthKM,n = 3)
...
da.bm<- dams%>% left_join(bmps, by=c("STATE"="StateAbbreviation")) 
joined<- st_join(da.bm,counties)
bmp.count<- joined %>% group_by(NAME10)%>% summarize(total_cost = sum(Cost, na.rm = TRUE))
bmp.cost<- st_join(counties,bmp.count)
tm_shape(bmp.cost) + tm_polygons(fill = "total_cost")

2.4 For each removed dam, find the closest stream segment

nearest.dam<- st_nearest_feature(dams,streams) %>% group_by(DAM_NAME)
dam.stream<- cbind(dams,streams[nearest.dam, ])

2.5 Calculate how many removed dams are (or were) in each state

dams.state<- dams%>% group_by(STATE)%>%summarize(num.dams=n())
